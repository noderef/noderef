...
import { getAlfrescoAuditApplicationsPath } from '../../../../lib/alfresco-endpoints.js';
...
export const auditAppsTool: ToolDefinition = {
  name: 'audit_apps',
  description: 'List registered Alfresco audit applications (GET /audit-applications).',
  skill: { kind: 'local_md', path: '../skills/audit_apps.md', version: 1 },
  inputSchema: {
    type: 'object',
    properties: {
  ...
  async execute(ctx: AgentExecutionContext, args: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      const path = getAlfrescoAuditApplicationsPath();
      const query = { skipCount, maxItems };
      const payload = await auditApi.listAuditApps(query);
