...
import { Paper, Stack, Table, Text, Anchor, Menu } from '@mantine/core';
...
interface NodeChildrenProps {
  childNodes: AlfrescoNodeDetails['children'];
  serverId: number;
}

export function NodeChildren({ childNodes, serverId }: NodeChildrenProps) {
  ...
  return (
    <Stack gap="md" p="md">
      <Paper withBorder p="md">
        {childNodes.length === 0 ? (
          <Text c="dimmed">{t('nodeBrowser:noChildren')}</Text>
        ) : (
          <Table>
            <Table.Thead>
              <Table.Tr>
                <Table.Th>{t('nodeBrowser:childName')}</Table.Th>
                <Table.Th>{t('nodeBrowser:childType')}</Table.Th>
                ...
                <Table.Th>{t('nodeBrowser:index')}</Table.Th>
              </Table.Tr>
            </Table.Thead>
            <Table.Tbody>
              {childNodes.map((child, idx) => (
                <Table.Tr key={idx}>
                  <Table.Td>
                    <Text size="sm">{child.name.prefixedName}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Text size="sm" c="dimmed">
                      {child.type.prefixedName}
                    </Text>
                  </Table.Td>
                  <Table.Td>
                    <Anchor
                      ...
                      onClick={() => handleNodeRefClick(child.nodeRef, child.name.prefixedName)}
                      onContextMenu={e =>
                        handleNodeRefContextMenu(e, child.nodeRef, child.name.prefixedName)
                      }
                    >
                      {child.nodeRef}
                    </Anchor>
                  ...
                  <Table.Td>
                    <Text size="sm" c="dimmed">
                      {child.assocType.prefixedName}
                    </Text>
                  </Table.Td>
                  <Table.Td>
                    <Text size="sm">{child.primary ? 'true' : 'false'}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Text size="sm">{child.index}</Text>
                  </Table.Td>
                </Table.Tr>
              ))}
            </Table.Tbody>
